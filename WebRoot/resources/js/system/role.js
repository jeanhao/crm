...
var roleColm = new Ext.grid.ColumnModel([
...
CRM.roleLook.role = Ext.extend(Ext.Panel, {
	...
	save : function() {
		if (this.fp.form.isValid()) {
			this.fp.form.submit({
				url : 'SysRole_saveorUpdate',
				method : 'POST',
				waitTitle : '请稍候',
				waitMsg : '正在保存......',
				success : function(form, action) {
					Ext.Msg.confirm('系统消息', action.result.msg+" 是否继续添加？", function(btn) {
						console.log(btn);
						if(btn == "no")
						{
							this.closeAddWin();
...
//		allRightStore.load();
	...
	initComponent : function() {
		...
		if(!this.flag){
			CRM.roleLook.role.prototype.flag = 0;
		}
		if(this.flag == 0){
			roleStore.load({
				params : {
			...
			CRM.roleLook.role.prototype.flag = 1;
		}
		this.add(roleGrid);
	}
});
